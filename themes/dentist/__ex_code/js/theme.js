			...
			plugins: function () {
				...
				if ($(pluginsCollection[3]).length) {
					$(pluginsCollection[3]).jackBox("init", {
						...
						baseName: Drupal.settings.basePath + "themes/esperia/plugins/jackbox",
						className: ".jackbox",
						useThumbs: true,
			...
			events: {
				...
				contactForm: function () {
					...
					cf.on("submit", function (event) {
						...
						var request = $.ajax({
							url: Drupal.settings.pathToTheme + "/templates/pages/contactsPage/sendMail.php",
							type: "post",
						...
						request.then(function (data) {
							...
							if (data == "1") {
								...
								if (cf.find('input').val() == "") {
									...
									text = "Inserisci almeno 20 caratteri nel messaggio!"
								}
								else if (miniCaptcha.length && +miniCaptcha.val() != 8) {
									text = "Risposta capcha errata!";
								}
